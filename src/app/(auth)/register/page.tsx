      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input 
            type="text" 
            {...register('name')}
            className={`w-full bg-surface-container-low border ${errors.name ? 'border-error' : 'border-outline-variant/30'} rounded-lg px-4 py-2 text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all`}
            placeholder="Alex Chen"
          />
          {errors.name && (
            <p className="mt-1 text-error text-[12px]">{errors.name.message}</p>
          )}
        </div>
        <div>
