...
export default function SpotlightCards({
  items = DEFAULT_ITEMS,
  subtext = "Everything you need to stop grinding mindlessly and start learning systematically.",
  heading = "Engineered for Technical Mastery",
  className,
}: SpotlightCardsProps) {
  ...
  return (
    <div
      className={cn(
        ...
        "bg-white dark:bg-[#06060f]/0", // transparent to let the ambient glow show
        className
      )}
    >
      ...
      <div className="relative mb-16 flex flex-col items-center text-center">
        {heading && (
          <h2 className="animate-fade-in-up font-heading-mobile md:text-[32px] md:leading-[40px] font-bold text-foreground mb-4">
            {heading}
          </h2>
        )}
        {subtext && (
          <p className="animate-fade-in-up delay-100 font-sans text-[16px] leading-[24px] text-muted-foreground max-w-2xl mx-auto">
            {subtext}
          </p>
        )}
      </div>
