...
export default function ActivityHeatmap({ data }: HeatmapProps) {
  ...
  return (
    <div className="w-full">
      {/* Header and buttons removed as requested (and to fix duplication) */}

      <div className="w-full pb-2 min-h-[140px] flex justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            {viewData.type === 'grid' ? (
              <div className="w-full flex justify-between gap-[1px] sm:gap-[2px] md:gap-1">
                {(viewData.data as any[]).map((week, wIndex) => (
                  <div key={wIndex} className="flex-1 min-w-0 flex flex-col gap-[1px] sm:gap-[2px] md:gap-1">
                    {wIndex % 4 === 0 && week[0] ? (
                      <span className="text-[9px] sm:text-[10px] text-muted-foreground h-4 mb-1 hidden sm:block truncate">{week[0].monthLabel}</span>
                    ) : (
                      <span className="h-4 mb-1 hidden sm:block"></span>
                    )}
                    {week.map((day: any) => (
                      <div 
                        key={day.label}
                        className={`w-full aspect-square rounded-[1px] sm:rounded-[2px] md:rounded-[3px] border ${getColor(day.count)} transition-all hover:scale-125 hover:z-10 relative group cursor-pointer`}
                      >
